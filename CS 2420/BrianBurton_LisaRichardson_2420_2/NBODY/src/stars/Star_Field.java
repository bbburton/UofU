...
//While you don't need to modify this code
//It should be noted, that there is one FIXME in the star_field class that you need to implement. 
//Read the comments in the file more more information.
...
public class Star_Field extends JPanel implements MouseListener, MouseWheelListener, ActionListener, MouseMotionListener
{
	...
	public void paintComponent( Graphics g )
	{
		...
		String fps = String.format("FPS: %.3f, Particles: %3d", this.gui_paint_counts / total_time, this.getComponentCount());

		String gcps = String.format("GCPS: %10.3f, Time to Calculate: %f", this.gravity_calculations / total_time, this.total_calc_time / this.gravity_calculations / 1000000000.0);
	...
	private void create_planets() {
		...
		Star sun = new Star(this.center_of_system.x, this.center_of_system.y, 0, 0, Solar_System_Facts.sun_mass, Solar_System_Facts.sun_radius, "Sun");

		Planet mercury = new Planet(Solar_System_Facts.mercury_distance, 0, 0, Solar_System_Facts.mercury_speed, Solar_System_Facts.mercury_mass, Solar_System_Facts.mercury_radius, "Mercury");
		Planet venus = new Planet(Solar_System_Facts.venus_distance, 0, 0, Solar_System_Facts.venus_speed, Solar_System_Facts.venus_mass, Solar_System_Facts.venus_radius, "Venus");
		Planet earth = new Planet(Solar_System_Facts.earth_distance, 0, 0, Solar_System_Facts.earth_speed, Solar_System_Facts.earth_mass, Solar_System_Facts.earth_radius, "Earth");
		Planet mars = new Planet(Solar_System_Facts.mars_distance, 0, 0, Solar_System_Facts.mars_speed, Solar_System_Facts.mars_mass, Solar_System_Facts.mars_radius, "Mars");
		Planet jupiter = new Planet(Solar_System_Facts.jupiter_distance, 0, 0, Solar_System_Facts.jupiter_speed, Solar_System_Facts.jupiter_mass, Solar_System_Facts.jupiter_radius, "Jupiter");
		Planet saturn = new Planet(Solar_System_Facts.saturn_distance, 0, 0, Solar_System_Facts.saturn_speed, Solar_System_Facts.saturn_mass, Solar_System_Facts.saturn_radius, "Saturn");
		...
		sun.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
		mercury.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
		venus.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
		earth.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
		mars.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
		jupiter.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
		saturn.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
	...
	public void add_planets_to_solar_system(int count) {
		...
		if (count >= 8) {
			...
			Black_Hole bh = new Black_Hole(-Solar_System_Facts.jupiter_distance, Solar_System_Facts.saturn_distance,Solar_System_Facts.sun_mass * 10, "Black Hole");
			bh.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
			this.add(bh, 0);
		}
	...
	public void create_flotsam(int how_many) {
		...
		for (int i = 0; i < how_many; i++) {
			...
			if (this.give_flotsam_initial_velocity) {
				...
				flotsam = new Flotsam(x, y, generator.nextInt(50000) - 25000, generator.nextInt(50000) - 25000); // velocity
			}
			...
			flotsam.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
			this.add(flotsam, 0);
		}
	}
	...
	public void update_positions() {
		...
		for (int i = 0; i < comps.length; i++) {
			...
			if (comps[i] instanceof Satellite) {
				...
				Satellite sat = (Satellite) comps[i];
				sat.update_screen_coordinates(this.center_of_system, this.radius_of_universe, this.getWidth(), this.getHeight());
			}
		}
	 ...
	 * 
	 * @param start -- if true, start simulating
	 */
	...
	{
		if(e.getSource() instanceof Star)
		{
			Component thing = (Component) e.getSource();
			((Star) thing).super_nova();
		}
	}
